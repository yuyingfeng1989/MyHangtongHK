 ...
 * 日历控件 功能：获得点选的日期区间
 */
public class ObdCalendarView extends View implements View.OnTouchListener {
    ...
    private boolean noTouchUp = false;
    private int isFirst = -1;

    public ObdCalendarView(Context context) {
        super(context);
        this.context = context;
        init();
    }

    public ObdCalendarView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        Log.d(TAG, "onDraw " + today);
        // 画框
        // canvas.drawPath(surface.boxPath, surface.borderPaint);
        for (int i = 1; i < 7; i++) {
            canvas.drawLine(0, surface.weekHeight + surface.cellHeight * i
                            + surface.cellHeight * 0.08f, surface.width,
                    surface.weekHeight + surface.cellHeight * i
                            + surface.cellHeight * 0.08f, surface.borderPaint);
        ...
        // 星期背景
        surface.cellBgPaint.setColor(getResources().getColor(R.color.obd_1b1b1b));
        canvas.drawRect(0, 0, surface.width, surface.weekHeight, surface.cellBgPaint);
        for (int i = 0; i < surface.weekText.length; i++) {
            float weekTextX = i
                    * surface.cellWidth
                    + (surface.cellWidth - surface.weekPaint
                    .measureText(context.getString(surface.weekText[i]))) / 2f;
            canvas.drawText(context.getString(surface.weekText[i]), weekTextX, weekTextY,
                    surface.weekPaint);
        }
        ...
        Log.v("tag", "curStartIndex " + curStartIndex + " " + curEndIndex
                + " todayIndex " + todayIndex);
        // 2016-6-17 18:14:46 tohere
        if (isFirst == -1)
        ...
        for (int i = 0; i < 42; i++) {
            ...
            if (isLastMonth(i)) {
                ...
                drawCellBg(canvas, i, surface.cellWhiteColor);
    ...
    private void drawCellText(Canvas canvas, int index, String text, int color) {
        ...
        surface.datePaint.setTypeface(font);
    ...
    private void drawCellBg(Canvas canvas, int index, int color) {
        ...
        //这个地方是为了解决一个bug
        if (surface.cellWhiteColor == color) {
            canvas.drawCircle((left + left + surface.cellWidth) / 2,
                    (top + top + surface.cellHeight) / 2 + 5,
                    (float) (surface.cellHeight / 3) * 1.1f,
                    surface.cellBgPaint);
        } else {
            canvas.drawCircle((left + left + surface.cellWidth) / 2,
                    (top + top + surface.cellHeight) / 2 + 6 * Utils.getDisplayMetrics(context) / 2,
                    (float) (surface.cellHeight / 3), surface.cellBgPaint);
        }
    }
    ...
    private void setSelectedDateByCoor(float x, float y) {
        ...
        Log.v("tag", "down index is " + downIndex + " curstartindex  "
                + curStartIndex + " curendindex " + curEndIndex
        ...
        if (todayIndex != -1 && downIndex > todayIndex) {
            noTouchUp = true;
            return;
        }
    ...
    private class Surface {
        ...
        public int bgColor = Color.parseColor("#b1b1b1");
        private int textColor = Color.GRAY;
        // private int textColorUnimportant = Color.parseColor("#666666");
        private int btnColor = Color.parseColor("#b1b1b1");
        private int borderColor = Color.parseColor("#b1b1b1");
        public int todayNumberColor = Color.BLACK;
        public int todayLaterColor = Color.parseColor("#b1b1b1");
        public int cellDownColor = Color.parseColor("#b1b1b1");
        public int cellWhiteColor =Color.parseColor("#b1b1b1");
        ...
        public void init() {
            ...
            borderWidth = 1f;// borderWidth < 1 ? 1 : borderWidth;

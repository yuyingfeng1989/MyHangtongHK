...
/**
 * @ClassName PercentLemon
 * @PackageName cn.changwentao.widget
 * @Description TODO
 * @Date 2016-5-19 下午2:33:08
 * @Version V1.0
 */
public class PercentLemon extends View {
     ...
     * 默认的中心圆颜色
     */
    private static final int DEFAULT_HEART_COLOR_NORMAL = 0xFF57c7c2;

    /**
     * 默认的中心圆点击颜色
     */
    private static final int DEFAULT_HEART_COLOR_CLICKED = 0xFF57c7c2;

    /**
     * 默认的外圆环起始颜色（百分比为0）
     */
    private static final int DEFAULT_SKIN_START_COLOR = 0xFFFFFFFF;

    /**
     * 默认的外圆环结束颜色（百分比为100）
     */
    private static final int DEFAULT_SKIN_END_COLOR = 0xFFFFFFFF;
     ...
     */
    private static final int DEFAULT_SKIN_DEPTH = 10;

    /**
     * 默认的外圆环颜色，百分比值之外的正常情况下圆环颜色
     */
    private static final int DEFAULT_SKIN_INVALID_COLOR = 0xFF96dfdc;
     ...
     * 描述画笔
     */
    private final TextPaint mDescribeTextPaint;
    ...
    public PercentLemon(Context context, AttributeSet attrs) {
        ...
        try {
            mTextColor = a.getColor(R.styleable.PercentLemon_centerTextColor,
                    DEFAULT_TEXT_COLOR);
            mDescribeColor = a.getColor(R.styleable.PercentLemon_describeColor,
                    DEFAULT_TEXT_COLOR);
            mDescribeTextPosition = a.getColor(R.styleable.PercentLemon_describeTextposition,
                    2);
            ...
            mSkinInvalidColor = a.getColor(
                    R.styleable.PercentLemon_defaultInvalidColor,
                    DEFAULT_SKIN_INVALID_COLOR);
            ...
            mPercentTextSize = a.getFloat(
                    R.styleable.PercentLemon_percentTextSize, 10f);
            mDescribeTextSize = a.getFloat(
                    R.styleable.PercentLemon_describeTextSize, 10f);
            sDescpribe = a.getString(R.styleable.PercentLemon_describeText);
            if (sDescpribe == null) {
                sDescpribe = "";
            }
            Log.d("tag", "percent text size is " + mPercentTextSize + " "
                    + mDescribeTextSize);
        } finally {
            a.recycle();
        }
        ...
        mDescribeTextPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
        mDescribeTextPaint.setColor(mDescribeColor);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Float.compare(mPercent, 0) >= 0) {
            ...
            setRawTextSize(mHeartRadius / 1.2f);
            canvas.drawText(percentText, mXCenter - (mTxtWidth) / 2, mYCenter
                    , mTextPaint);


            mTextPaint.setTextSize(mPercentTextSize * density);
            mDescribeTextPaint.setTextSize(mDescribeTextSize * density);
            Log.d("tag", "percent text size final " + mPercentTextSize + " "
                    + mDescribeTextSize + " " + density);
            float mTxtDescribeWidth = mDescribeTextPaint.measureText(
                    sDescpribe, 0, sDescpribe.length());
            Rect bounds = new Rect();
            mDescribeTextPaint.getTextBounds(sDescpribe, 0, sDescpribe.length(), bounds);
            canvas.drawText(sDescpribe, mXCenter - mTxtDescribeWidth / 2,
                    mYCenter + 2.0f * bounds.height(), mDescribeTextPaint);
     ...
     * 分数描述
     */
    public void setDescribeText(String descpribe) {
        sDescpribe = descpribe;
        invalidate();
    }
